 ...
 */
inline double weightPoly(const double r, const double r0, const double c,
                         const double m) {
  if (r > r0) {
    return 0;
  }
 ...
 * w(r) = c / (d + (r/r0)^m)
 */
inline double weightPow(const double r, const double r0, const double c,
                        const double d, const double m) {
  // const double rr0 = r / r0;
  return c / (d + pow(r / r0, m));
}
/**
 * Exponential weighting of the form:
 * w(r) = c / (d + e^(-r/r0))
 */
inline double weightExp(const double r, const double r0, const double c,
                        const double d) {
  // const double rr0 = r / r0;
  return c / (d + exp(-r / r0));
}
...
void getWeights(const int size, double *r1s, const double *r2s,
                const bool squared, const pybind11::dict &weighting,
                double *weights);
void getWeights(const int size, const double *r1s,
                const pybind11::dict &weighting, double *weights);
