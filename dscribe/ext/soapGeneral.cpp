...
namespace SOAPconstants
{
    ...
    constexpr std::array<double, 100> ws = {
...
{
    return SOAPconstants::factorList.data();
}

const double *getws()
{
    return SOAPconstants::ws.data();
}
...
double *getYlmi(double *x, double *y, double *z, double *oOri, int icount, int lMax)
{
    ...
    for (int l = 0; l < lMax + 1; ++l)
    {
        for (int m = 0; m < l + 1; ++m)
        {
            for (int i = 0; i < icount; ++i)
            {
                Ylmi[2 * (lMax + 1) * icount * l + 2 * icount * m + 2 * i] = factorY(l, m) * legPol[icount * (lMax + 1) * l + icount * m + i] * ChiCos[m * icount + i];
                Ylmi[2 * (lMax + 1) * icount * l + 2 * icount * m + 2 * i + 1] = factorY(l, m) * legPol[icount * (lMax + 1) * l + icount * m + i] * ChiSin[m * icount + i];
            }
        }
    }
    free(legPol);
...
void getC(double *C, double *rw2, double *gns, double *summed, double rCut, int lMax, int rsize, int gnsize, int nCenters, int nNeighbours, double eta, double *weights)
{
    // Initialize to zero
    ...
    for (int n = 0; n < gnsize; ++n)
    {
        ...
        if (nCenters > 0)
        {
            ...
            for (int iCenter = 0; iCenter < nCenters; ++iCenter)
            {
                for (int rw = 0; rw < rsize; ++rw)
                {
                    C[2 * (lMax + 1) * (lMax + 1) * n] += weight * 0.5 * 0.564189583547756 * rw2[rw] * SOAPconstants::ws[rw] * gns[rsize * n + rw] * exp(-eta * rw2[rw]);
                }
            }
        }
        for (int l = 0; l < lMax + 1; ++l)
        {
            for (int m = 0; m < l + 1; ++m)
            {
                for (int rw = 0; rw < rsize; ++rw)
                {
                    C[2 * (lMax + 1) * (lMax + 1) * n + l * 2 * (lMax + 1) + 2 * m] += rw2[rw] * SOAPconstants::ws[rw] * gns[rsize * n + rw] * summed[2 * (lMax + 1) * l * rsize + 2 * m * rsize + 2 * rw];         // Re
                    C[2 * (lMax + 1) * (lMax + 1) * n + l * 2 * (lMax + 1) + 2 * m + 1] += rw2[rw] * SOAPconstants::ws[rw] * gns[rsize * n + rw] * summed[2 * (lMax + 1) * l * rsize + 2 * m * rsize + 2 * rw + 1]; //Im
                }
            }
        }
    }
                        ...
                        for (int N1 = 0; N1 < nMax; N1++)
                        {
                            for (int N2 = N1; N2 < nMax; N2++)
                            {
                                ...
                                for (int m = 0; m < l + 1; ++m)
                                {
                                    ...
                                    {
                                        sum += 2 * (Cs[i * nCoeffs + 2 * Z1 * (lMax + 1) * (lMax + 1) * nMax + 2 * (lMax + 1) * (lMax + 1) * N1 + l * 2 * (lMax + 1) + 2 * m] * Cs[i * nCoeffs + 2 * Z2 * (lMax + 1) * (lMax + 1) * nMax + 2 * (lMax + 1) * (lMax + 1) * N2 + l * 2 * (lMax + 1) + 2 * m] + Cs[i * nCoeffs + 2 * Z1 * (lMax + 1) * (lMax + 1) * nMax + 2 * (lMax + 1) * (lMax + 1) * N1 + l * 2 * (lMax + 1) + 2 * m + 1] * Cs[i * nCoeffs + 2 * Z2 * (lMax + 1) * (lMax + 1) * nMax + 2 * (lMax + 1) * (lMax + 1) * N2 + l * 2 * (lMax + 1) + 2 * m + 1]);
                                    }
                                }
    ...
    CellList cellList)
{
    ...
    const int nFeatures = crossover ? (Nt * nMax) * (Nt * nMax + 1) / 2 * (lMax + 1) : Nt * (lMax + 1) * ((nMax + 1) * nMax) / 2;
    ...
    constexpr int rsize = 100; // The number of points in the radial integration grid
    ...
    for (int i = 0; i < Hs; ++i)
    {
        ...
        for (const auto &ZIndexPair : atomicTypeMap)
        {
            ...
            Ylmi = getYlmi(dx, dy, dz, oOri, nNeighbours, lMax);
            summed = getIntegrand(Flir, Ylmi, rsize, nNeighbours, lMax, weights);

            getC(C, rw2, gss, summed, rCut, lMax, rsize, nMax, nCenters, nNeighbours, eta, weights);
